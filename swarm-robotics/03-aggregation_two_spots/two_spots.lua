...
function aggregationLayer()
    ...
    if IS_MOVING == true then
        STOPPING_PROB = math.min(MAX_STOPPPING_PROB, SPONTANEOUS_STOPPING_PROB + ALFA * N + (spot * D_S))
        ...
        WALKING_PROB = math.max(MIN_WALKING_PROB, SPONTANEOUS_WALKING_PROB - BETA * N - (spot * D_W))
